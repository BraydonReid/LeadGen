...
export default function Hero({ totalLeads, consumerIntentCount }: { totalLeads: number; consumerIntentCount?: number }) {
  const formatted = totalLeads > 0 ? totalLeads.toLocaleString() : "60,000+";
  const intentFormatted = consumerIntentCount && consumerIntentCount > 0 ? consumerIntentCount.toLocaleString() : "1,000+";
  ...
  return (
    <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 text-white">
      <div className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 bg-blue-500/20 border border-blue-400/30 rounded-full px-4 py-1.5 text-sm text-blue-300 mb-8">
            ...
            Fresh leads added daily · 34 states covered
          </div>
      ...
      <div className="border-t border-white/10 bg-black/20">
        <div className="max-w-6xl mx-auto px-6 py-6 grid grid-cols-2 md:grid-cols-6 gap-6 text-center">
          {[
            { value: formatted, label: "Total leads" },
            { value: "98%", label: "Phone verified" },
            { value: intentFormatted, label: "Intent leads" },
            { value: "93", label: "Industries" },
