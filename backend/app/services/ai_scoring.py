...
async def score_lead_batch(db: AsyncSession, batch_size: int = 50) -> int:
    ...
    for lead in leads:
        try:
            ...
            # Never let a single lead failure block the batch
            logger.warning(f"[ai_scoring] Failed to score lead {lead.id}: {e}")
            # Mark as scored with a neutral score so it won't be retried indefinitely
            await db.execute(
                update(Lead)
                .where(Lead.id == lead.id)
                .values(conversion_score=50, ai_scored_at=func.now())
            )
